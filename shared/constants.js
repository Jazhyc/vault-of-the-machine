...
export const BOTS = {
  ...
  // missile hunting IT (sprint + fire is the designed counterplay). Missiles
  // hunting a HUMAN are that human's game — one shoot-down chain-cooks a
  // converging salvo, so ANY reliable echo interception (tested at every
  // range/limit) deletes whole waves and the mechanic never threatens
  // anyone. The squad therefore peels only as a clutch save: ONE interceptor,
  // only when the hunted human is wounded below seekerSaveHp of max, only
  // inside seekerAssistR of them. seekerNotice delays the first reaction past
  // the pop arc (0.9 s); seekerAcc taxes tracking a darting warhead.
  seekerNotice: 1.6, seekerAssistR: 14, seekerSaveHp: 0.5, seekerAcc: 0.65,
